...
func (s *Scanner) scanToken() {
	...
	switch c {
		// Symbols
		case '(': s.addToken(token.LEFT_PAREN);
		case ')': s.addToken(token.RIGHT_PAREN);
		case '{': s.addToken(token.LEFT_BRACE);
		case '}': s.addToken(token.RIGHT_BRACE);
		case ',': s.addToken(token.COMMA);
		case '.': s.addToken(token.DOT);
		case '-': s.addToken(token.MINUS);
		case '+': s.addToken(token.PLUS);
		case ';': s.addToken(token.SEMICOLON);
		case '*': s.addToken(token.STAR);
		case '!': s.addTokenConditional('=', token.BANG_EQUAL, token.BANG);
		case '=': s.addTokenConditional('=', token.EQUAL_EQUAL, token.EQUAL);
		case '<': s.addTokenConditional('=', token.LESS_EQUAL, token.LESS);
		case '>': s.addTokenConditional('=', token.GREATER_EQUAL, token.GREATER);
		case '/': {
			if s.match('/') {
